...
func upload(w http.ResponseWriter, r *http.Request) {
	...
	for _, v := range fhs {
		...
		mf, e := os.OpenFile(storagePath+path+v.Filename, os.O_WRONLY|os.O_CREATE, 0666)
		if e != nil {
			fmt.Println(e)
			fmt.Fprint(w, e.Error())
			return
...
func SetClipboard(str string) {
	...
	if len(clipConns) > 0 {
		b, _ := json.Marshal(Msg{Message: str, State: "OK"})
...
func wsClipboard(ws *websocket.Conn) {
	...
	for {
		...
		mEventHandler.OnClipboardReceived(cm.Message, false)
		homeData.Clipboard = cm.Message
	}
}
