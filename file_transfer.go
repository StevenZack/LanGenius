...
<script>function initMe(){if("zh-CN"==navigator.language){null!=document.getElementById("btCopy")&&(document.getElementById("btCopy").value="复制"),null!=document.getElementById("btSend")&&(document.getElementById("btSend").value="发送"),null!=document.getElementById("txtClipboard")&&(document.getElementById("txtClipboard").innerHTML="剪切板"),null!=document.getElementById("txtBindClip")&&(document.getElementById("txtBindClip").innerHTML="绑定"),document.getElementById("txtFiles").innerHTML="文件",document.getElementById("liveStreamPage").innerHTML="直播页面",window.Succeed="成功",document.getElementById("submit_button").value="上传文件";for(var e=document.getElementsByClassName("downloadA"),n=0;n<e.length;n++)e[n].innerHTML="下载"}else window.Succeed=" Succeed";"checked"==getCookie("LanGenius-Bind-Clipboard")&&(document.getElementById("btBind").checked=!0,BindClipboard()),detectLink()}function detectLink(){if(null!=document.getElementById("cb")){var e=document.getElementById("cb").value,n="";if(e.indexOf("http://")>-1){var t=e.indexOf(" ",e.indexOf("http://"));n=e.substring(e.indexOf("http://"),-1==t?e.length:t)}else if(e.indexOf("https://")>-1){t=e.indexOf(" ",e.indexOf("https://"));n=e.substring(e.indexOf("https://"),-1==t?e.length:t)}""!=n?("zh-CN"==navigator.language?document.getElementById("openUrl").innerHTML="打开网址":document.getElementById("openUrl").innerHTML="Open URL",document.getElementById("openUrl").href=n):document.getElementById("openUrl").innerHTML=""}}function setCookie(e,n,t){var d=new Date;d.setTime(d.getTime()+24*t*60*60*1e3);var o="expires="+d.toGMTString();document.cookie=e+"="+n+"; "+o}function getCookie(e){for(var n=e+"=",t=document.cookie.split(";"),d=0;d<t.length;d++){var o=t[d].trim();if(0==o.indexOf(n))return o.substring(n.length,o.length)}return""}function copy(){detectLink();var e=document.getElementById("btCopy");document.getElementById("cb").select(),document.execCommand("Copy");var n=e.value;e.value=window.Succeed,e.disabled="disabled",setTimeout("document.getElementById('btCopy').value='"+n+"';document.getElementById('btCopy').disabled=''",1e3)}function send(){var e=document.getElementById("btSend"),n=e.value,t=document.getElementById("cb").value,d=new XMLHttpRequest,o=new FormData,i=new Object;i.Cb=t,o.append("cb",JSON.stringify(i)),d.onload=function(t){200!=this.status&&302!=this.status&&304!=this.status||(e.value=window.Succeed,e.disabled="disabled",setTimeout("document.getElementById('btSend').value='"+n+"';document.getElementById('btSend').disabled='';",1e3))},d.open("POST","/send",!0),d.send(o),console.log("sent clip")}function DoUpload(){for(var e=new FormData,n=document.getElementById("myUploadFile"),t=0;t<n.files.length;t++)e.append("myUploadFile",n.files[t]);uploading();var d=new XMLHttpRequest;d.upload.addEventListener("progress",function(e){if(e.lengthComputable){var n=Math.round(100*e.loaded/e.total);document.getElementById("uploadProgress").value=n,document.getElementById("result").innerHTML=n.toString()+"%"}else document.getElementById("result").innerHTML="unable to compute"},!1),d.onreadystatechange=function(){4==d.readyState&&200==d.status&&(document.getElementById("result").innerHTML=d.responseText,setTimeout("uploadDone()",1e3))},d.addEventListener("load",function(e){document.getElementById("result").innerHTML=e.target.responseText,setTimeout("uploadDone()",1e3)},!1),d.addEventListener("error",function(e){document.getElementById("result").innerHTML="Something went wrong .",setTimeout("uploadDone()",1e3)},!1),d.addEventListener("abort",function(e){document.getElementById("result").innerHTML="Abort .",setTimeout("uploadDone()",1e3)},!1),d.open("POST","/upload"),d.send(e)}function uploading(){"zh-CN"==navigator.language?document.getElementById("submit_button").value="上传中...":document.getElementById("submit_button").value="Uploading",document.getElementById("submit_button").disabled="disabled",document.getElementById("myUploadFile").disabled="disabled"}function uploadDone(){document.getElementById("uploadProgress").value=0,"zh-CN"==navigator.language?document.getElementById("submit_button").value="上传文件":document.getElementById("submit_button").value="Upload",document.getElementById("submit_button").disabled="",document.getElementById("myUploadFile").disabled=""}function BindClipboard(){document.getElementById("btBind").checked?(setCookie("LanGenius-Bind-Clipboard","checked",365),window.wsBind=new WebSocket("ws://"+location.href.split("/")[2]+"/wsClipboard"),window.wsBind.onmessage=function(e){var n=JSON.parse(e.data);document.getElementById("cb").value=n.Content,copy()},window.wsBind.onopen=function(){console.log("bind open")},window.wsBind.onerror=function(e){console.log("bind err:"+e.data)},window.wsBind.onclose=function(){console.log("bind closed"),window.wsBind=null}):(setCookie("LanGenius-Bind-Clipboard","unchecked",365),null!=window.wsBind&&(window.wsBind.close(),window.wsBind=null))}function clipOnChange(e){if(console.log("onchange"),detectLink(),null!=window.wsBind){var n=new Object;n.Content=e.value,window.wsBind.send(JSON.stringify(n)),console.log("bind sent")}}setTimeout("initMe()",50)</script>
